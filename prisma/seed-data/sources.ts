...
export const SOURCES = [
    ...
    // ===== NETHERLANDS =====
    {
      ...
      config: {
        ...
        // On/after run-day the site flips both pages to a "NO RUN TODAY"
        ...
        // the title/location. Drop the placeholder at ingest. Prod showed two
        // variants: the bare "NO RUN TODAY" and the run-day "AH3 #1478 — NO RUN
        // TODAY" (the adapter prefixes "AH3 #N — " when the page still carries a
        // run id) — kept as two anchored rules rather than one optional-prefix
        // regex because nesting `\s*` inside a `(?:…)?` group trips safe-regex2
        // (star height 2) and compileSilentSkipRules would silently drop it.
        // Unconditional — the placeholder is never a real trail, so a run number
        // must NOT rehabilitate it (#1799-followup; cf. Hibiscus no-run rule).
        silentlySkipPatterns: [
          { pattern: String.raw`^NO RUN TODAY$`, field: "title" },
          { pattern: String.raw`^AH3\s*#\s*\d+\s*[—–-]\s*NO RUN TODAY$`, field: "title" },
        ],
      },
      kennelCodes: ["ah3-nl"],
    },
    ...
    // future feed (reconcile.ts timeMin guard). Every source owns a historical
    // backfill (scripts/backfill-<code>-history.ts). GUID is the stable filter.
    {
      name: "Brazil Nuts H3 Harrier Central",
      url: "https://harriercentralpublicapi.azurewebsites.net/api/PortalApi/",
      type: "HARRIER_CENTRAL" as const,
      trustLevel: 8, scrapeFreq: "daily", scrapeDays: 365,
      config: {
        publicKennelId: "10b3befa-4b38-49b5-b68b-95ad07d4b087",
        defaultKennelTag: "bnh3",
        defaultTitle: "Brazil Nuts H3",
        staleTitleAliases: ["Placeholder event for BNH3"],
        upcomingOnly: true,
      },
      kennelCodes: ["bnh3"],
    },
    {
      name: "Newcastle H3 Harrier Central",
      url: "https://harriercentralpublicapi.azurewebsites.net/api/PortalApi/",
      type: "HARRIER_CENTRAL" as const,
      trustLevel: 8, scrapeFreq: "daily", scrapeDays: 365,
      config: {
        publicKennelId: "f1266de7-9bdc-4cb7-95ca-4846fd7aa01c",
        defaultKennelTag: "newcastle-h3",
        defaultTitle: "Newcastle H3",
        staleTitleAliases: ["Placeholder event for NH3-GB"],
        upcomingOnly: true,
      },
    ...
    },
    {
      ...
      trustLevel: 8, scrapeFreq: "daily", scrapeDays: 365,
      config: {
        publicKennelId: "79557c23-c531-4499-b5c2-36fe19af484c",
        defaultKennelTag: "aberdeen-h3",
        defaultTitle: "Aberdeen H3",
        staleTitleAliases: ["Placeholder event for AH3"],
        upcomingOnly: true,
      },
      kennelCodes: ["aberdeen-h3"],
    },
    {
      name: "Plympton H3 Harrier Central",
      url: "https://harriercentralpublicapi.azurewebsites.net/api/PortalApi/",
      type: "HARRIER_CENTRAL" as const,
      trustLevel: 8, scrapeFreq: "daily", scrapeDays: 365,
      config: {
        publicKennelId: "901daedc-2ce5-47d8-9e63-e4c3cd9135da",
        defaultKennelTag: "plympton-h3",
        defaultTitle: "Plympton H3",
        staleTitleAliases: ["Placeholder event for PH3"],
        upcomingOnly: true,
      },
      kennelCodes: ["plympton-h3"],
    },
    {
      name: "Toulouse H3 Harrier Central",
      url: "https://harriercentralpublicapi.azurewebsites.net/api/PortalApi/",
      type: "HARRIER_CENTRAL" as const,
      trustLevel: 8, scrapeFreq: "daily", scrapeDays: 365,
      config: {
        publicKennelId: "9fa2eab8-a905-490a-a509-23b5c98e6061",
        defaultKennelTag: "toulouse-h3",
        defaultTitle: "Toulouse H3",
        staleTitleAliases: ["Placeholder event for TH3-FR"],
        upcomingOnly: true,
      },
      kennelCodes: ["toulouse-h3"],
    },
    {
      name: "The New Town Hash Harrier Central",
      url: "https://harriercentralpublicapi.azurewebsites.net/api/PortalApi/",
      type: "HARRIER_CENTRAL" as const,
      trustLevel: 8, scrapeFreq: "daily", scrapeDays: 365,
      config: {
        publicKennelId: "594bea0c-35fb-4259-9207-a59de10e6935",
        defaultKennelTag: "tnth3",
        defaultTitle: "The New Town Hash",
        staleTitleAliases: ["Placeholder event for TNTH3"],
        upcomingOnly: true,
      },
      kennelCodes: ["tnth3"],
    },
    {
      name: "DAFT H3 Harrier Central",
      url: "https://harriercentralpublicapi.azurewebsites.net/api/PortalApi/",
      type: "HARRIER_CENTRAL" as const,
      trustLevel: 8, scrapeFreq: "daily", scrapeDays: 365,
      config: {
        publicKennelId: "9100fc6d-859a-4e8b-ba74-54c11a908939",
        defaultKennelTag: "dafth3",
        defaultTitle: "DAFT H3",
        staleTitleAliases: ["Placeholder event for DAFTH3"],
        upcomingOnly: true,
      },
      kennelCodes: ["dafth3"],
    },
    {
      name: "FILTH H3 Harrier Central",
      url: "https://harriercentralpublicapi.azurewebsites.net/api/PortalApi/",
      type: "HARRIER_CENTRAL" as const,
      trustLevel: 8, scrapeFreq: "daily", scrapeDays: 365,
      config: {
        publicKennelId: "4483b884-e862-44ca-b52a-d95c9e876d4b",
        defaultKennelTag: "filth-h3",
        defaultTitle: "FILTH H3",
        staleTitleAliases: ["Placeholder event for FILTH"],
        upcomingOnly: true,
      },
      kennelCodes: ["filth-h3"],
    },
    {
      name: "Sembach H3 Harrier Central",
      url: "https://harriercentralpublicapi.azurewebsites.net/api/PortalApi/",
      type: "HARRIER_CENTRAL" as const,
      trustLevel: 8, scrapeFreq: "daily", scrapeDays: 365,
      config: {
        publicKennelId: "b19170aa-1cad-4c14-bee6-232d3061779d",
        defaultKennelTag: "sembach-h3",
        defaultTitle: "Sembach H3",
        staleTitleAliases: ["Placeholder event for SH3-DE"],
        upcomingOnly: true,
      },
      kennelCodes: ["sembach-h3"],
    },
    {
      name: "BeerSpoke H3 Harrier Central",
      url: "https://harriercentralpublicapi.azurewebsites.net/api/PortalApi/",
      type: "HARRIER_CENTRAL" as const,
      trustLevel: 8, scrapeFreq: "daily", scrapeDays: 365,
      config: {
        publicKennelId: "2e2c1cd6-9c66-4e95-819d-91aeddf1fef1",
        defaultKennelTag: "beerspoke-h3",
        defaultTitle: "BeerSpoke H3",
        staleTitleAliases: ["Placeholder event for BSH3"],
        upcomingOnly: true,
      },
      kennelCodes: ["beerspoke-h3"],
    },
    ...
    //
    // Run-number anchor (#2105, same mechanism as PFH3 #2043): Run #1883 ran
    // Sun 2026-05-17 per the public Budapest H3 Google Group
    // (groups.google.com/g/budapesthhh). Runs #1859 (2025-11-30) → #1883 are
    // exactly weekly-consecutive, so the anchor extrapolates across the active
    // window. Both rows share the SAME anchor so numbering stays continuous
    // across the Apr/Nov season boundary (each Sunday is generated by exactly one
    // row; computeRunNumber is purely date-based). WEEKLY INTERVAL=1 → date
    // generation ignores anchorDate, so this only adds run numbers.
    //
    // URL note (#2104): budapesthashhouseharriers.org is NXDOMAIN. STATIC_SCHEDULE
    // never fetches the url (cosmetic only), so events still generate — repointed
    // to the live (private) FB group. The kennel's only public presence is that
    // private group, so no scrapable FB source is possible. Kennel.website still
    // carries the dead .org URL (kennels.ts) — left for the profile owner.
    {
      name: "Budapest H3 Static Schedule (Summer Sundays)",
      url: "https://www.facebook.com/groups/1014178591932185",
      type: "STATIC_SCHEDULE" as const,
      trustLevel: 3,
      scrapeFreq: "weekly",
      scrapeDays: 90,
    ...
    },
    {
      ...
      config: {
        ...
        // Same anchor as the Summer row — keeps run numbers continuous across
        // the season boundary (see the Summer row's note above).
        anchorDate: "2026-05-17",
        startRunNumber: 1883,
        startTime: "10:30",
        defaultTitle: "Budapest H3 Weekly Run",
