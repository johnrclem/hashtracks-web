...
- **Scraping:** HTTP fetch + Cheerio for static HTML; NAS-hosted stealth headless Chrome (Playwright + `playwright-extra`/`puppeteer-extra-plugin-stealth` on external NAS, not in the app) for JS-rendered sites (Wix, Google Sites) and Cloudflare-protected sites via `browserRender()` — auto-clears Cloudflare "Just a moment…" Bot Fight Mode challenges with a per-hostname `cf_clearance` cookie cache; Blogger API v3 for Blogspot-hosted sites; GenericHtmlAdapter for config-driven CSS selector scraping; STATIC_SCHEDULE adapter for RRULE-based event generation; Meetup public REST API adapter; Harrier Central public REST API adapter (hashruns.org, config-driven); FACEBOOK_HOSTED_EVENTS adapter for public FB Pages with `/upcoming_hosted_events` tab (SSR'd inline GraphQL)
...
**Rules:**
- Migration files are **immutable** once applied — editing them breaks the checksum and blocks deploys. To undo a change, add a new migration.
- `prisma/manual-sql/` is legacy — new destructive ops must be expressed as migrations. Partial indexes and other constructs Prisma can't express in `schema.prisma` should be added as hand-written migration files (see `prisma/migrations/*_audit_suppression_global_unique/`).
