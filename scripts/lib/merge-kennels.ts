...
export async function mergeKennels(config: MergeConfig): Promise<void> {
  ...
  const sourceKennel = await prisma.kennel.findFirst({
    where: { shortName: config.sourceShortName },
    include: {
      _count: {
        select: {
          events: true,
  ...
  const targetKennel = await prisma.kennel.findFirst({
    where: { shortName: config.targetShortName },
    select: { id: true, shortName: true, slug: true },
  });

  if (!sourceKennel) {
    console.log(`✅ ${config.sourceShortName} kennel not found (already merged?)`);
    return;
  }
  ...
  const existingEventDates = await prisma.event.findMany({
    where: {
      ...
      date: { in: eventDates.map((e: { date: Date }) => e.date) },
    },
    select: { date: true },
  });
  ...
  if (existingEventDates.length > 0) {
    ...
    existingEventDates.forEach((e: { date: Date }) =>
  ...
  console.log(`\n${config.targetShortName} now has ${finalEvents} events`);
