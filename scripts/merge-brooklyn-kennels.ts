...
async function resolveHasherDuplicates(prisma: any, sourceKennelId: string, targetKennelId: string) {
  ...
  for (const hasher of hashers) {
    const existing = await prisma.kennelHasher.findFirst({
      where: { kennelId: targetKennelId, hashName: { equals: hasher.hashName, mode: "insensitive" } },
    });
    if (!existing) continue;
    ...
    if (sourceComplete > targetComplete) {
      await prisma.kennelHasher.update({
        ...
        data: { nerdName: hasher.nerdName || existing.nerdName, email: hasher.email || existing.email, phone: hasher.phone || existing.phone, notes: hasher.notes || existing.notes },
      });
...
async function main() {
  ...
  const targetKennel = await prisma.kennel.findFirst({
    where: { shortName: "BrH3" },
    select: { id: true, shortName: true, slug: true },
  });

  if (!sourceKennel) {
    console.log("✅ Brooklyn H3 kennel not found (already merged?)");
    return;
  }

  if (!targetKennel) {
    throw new Error("BrH3 kennel not found!");
  }
  ...
  console.log(`\nBrH3 now has ${finalEvents} events`);
