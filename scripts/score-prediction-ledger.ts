 ...
 *
 * Reads the PredictionSnapshot + LedgerObservation tables the weekly cron
 * (/api/cron/prediction-ledger) accumulates and reports drift-proof forward
 * calibration:
 *   - PRECISION per confidence × actual-days-out bin, over HIT/MISS rows only
 *     (PRECONFIRMED + UNOBSERVED excluded — contamination / unobserved, not model signal).
 *   - RECALL from the LedgerObservation census (population independent of predictions),
 *     so never-predicted kennels' real runs count as false negatives.
 *   - Pending inventory + the date the first matured scores arrive.
 *
 * Until cohorts mature (~weeks/months after the cron starts) most rows are PENDING,
 * so precision/recall are sparse by design. Output: docs/audits/prediction-ledger-<date>.md.
 *
...
async function run(prisma: PrismaClient): Promise<void> {
  ...
  const observations = await prisma.ledgerObservation.count();
  ...
  const md: string[] = [
    ...
    "",
    `Snapshots: **${total}** · Observation-census rows: **${observations}**` +
  ...
  if (scored === 0) {
    ...
    md.push(
      "",
      "Recall (from the LedgerObservation census) populates once matured cohorts have an observable",
      "denominator; see the pipeline doc. PRECONFIRMED + UNOBSERVED are excluded from both metrics.",
      "",
    );
  }
  ...
  const { mdPath } = writeAuditReport(`prediction-ledger-${date}`, md.join("\n"), {
    generatedAt: new Date().toISOString(), total, observations, byOutcome,
