...
async function main() {
  ...
  const sourceKennel = await prisma.kennel.findFirst({
    where: { shortName: "Harriettes (NYC)" },
    include: {
      _count: {
        select: {
          events: true,
  ...
  const targetKennel = await prisma.kennel.findFirst({
    where: { shortName: "Harriettes" },
    select: { id: true, shortName: true, slug: true },
  });

  if (!sourceKennel) {
    console.log("✅ Harriettes (NYC) kennel not found (already merged?)");
    return;
  }

  if (!targetKennel) {
    throw new Error("Harriettes kennel not found!");
  }
  ...
  console.log(`\nHarriettes now has ${finalEvents} events`);
