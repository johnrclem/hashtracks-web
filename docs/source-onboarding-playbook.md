...
71. **Weebly/CMS sites use `<br>` for line breaks inside inline elements** — Don't use Cheerio's `.text()` or `.children().each()` on Weebly/CMS pages. These platforms render multiple labeled fields inside a single parent element (e.g., `<h2>`) using `<br>` for visual line breaks, not separate child elements. Neither `.text()` (drops `<br>`) nor `.children()` (gets the whole `<strong>` blob) works. Instead, process `innerHTML` as a string: collapse all whitespace → convert `<br>` to `\n` → insert spaces after inline closing tags → strip remaining HTML. See `extractWeeblyBlockText()` in `edinburgh-h3.ts` for the pattern.
