...
- **Datacenter-IP checkpoint resilience (#1939):** FB serves a checkpoint / "content isn't available" wall (HTTP 200, zero event nodes) to some Pages from datacenter IPs. The adapter fetches direct, then retries through the NAS residential proxy when the body `looksLikeFbBlock` or the request 403s/throws; `config.useResidentialProxy: true` forces proxy-first for chronically-blocked Pages. A still-blocked result becomes a FAILED scrape, never a silent 0-events. The checkpoint is IP-specific and not reproducible off a datacenter IP.
- **Past-events backfill (#1940):** `config.includePastEvents: true` additionally ingests `/past_hosted_events` (listing-tab fields only, best-effort, deduped by event id, bounded by `pastWindowDays` default 365). This stays compatible with the `upcomingOnly` invariant: reconcile remains future-clamped, so past events are pure enrichment (a real FB title overwrites a STATIC_SCHEDULE placeholder) and are never cancellation candidates. Enabled on all FB source rows.
