...
| 10 | Brasília H3 | Brasilia Hash House Harriers | Brasília, Brazil | HTML_SCRAPER (Blogspot — Blogger API, mirror `ofh3.ts`/`hangover.ts`) | `brasiliah3.blogspot.com` (`/feeds/posts/default?alt=json`) | **High — verified live 2026-06-03** | no `brasilia*`/Brazil slug (sitemap 423) | queued | 🇧🇷 **First Brazil kennel.** Founded **1989** (og:description "since 1989"). Latest **Hash N+340 "Praça dos Orixás" for Sun 7 Jun 2026** (posted 2 Jun, yesterday); biweekly Sunday. ⚠️ **Run numbers are `N+NNN` format** (e.g. "Hash N+340") — extract the integer 340. Title carries run #; body has Hare, Start location, "Sunday, Xth of Month" date (prose-heavy, AI-recovery may help). Deep archive back to **Apr 2019** (~2 posts/mo, Blogger API paginates) → one-shot backfill candidate. kennelCode `brasilia-h3` (`bh3` taken). Add Brazil COUNTRY+Brasília METRO (5-edit region.ts). |
