...
- **Sort multi-value joined fields (hares, tags, scribes)** before `join(", ")` — nondeterministic API row order otherwise produces fresh fingerprints per scrape and breaks idempotency. Seletar re-run inserted 74 dup RawEvents before PR #541 fixed it with `[...names].sort(...).join(", ")`.
