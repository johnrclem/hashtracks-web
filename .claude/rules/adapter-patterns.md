...
- **`src/adapters/utils.ts`** — `extractHashRunNumber` (`#NNN` parser with delimiter guard, see PR #1147 / [memory](reference_extract_hash_run_number_helper.md)), `applyWeekdayShift` (pure helper for shifting dates onto a target weekday — see #1123), date utilities, field extraction.
- **`src/adapters/hare-extraction.ts`** — `extractHares`, `DEFAULT_HARE_PATTERNS`, `PHONE_TRAILING_RE`. Lifted out of the GCal adapter in #1215; both Meetup and Phoenix HHH now import from here.
...
- **`src/adapters/gohash-platform.ts`** — shared parser for goHash.app SaaS sites (Penang H3, HHH Penang).
- **`src/adapters/yii-hareline.ts`** — shared parser for Yii GridView hareline pages (Petaling H3, KL Full Moon).
