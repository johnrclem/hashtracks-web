...
- **FB hosted_events SSR shape rotates; the parser must pin to a small field surface** — The FACEBOOK_HOSTED_EVENTS adapter parses the GraphQL JSON island in `<script type="application/json">` tags. FB rotates the shape periodically (acknowledged in `docs/facebook-integration-strategy.md`); pinning to a small set of fields keeps the parser robust. Listing-tab fields consumed today: `__typename:Event`, `id`, `name`, `start_timestamp`, `is_canceled`, `event_place.contextual_name`, `event_place.location.latitude`, `event_place.location.longitude`. Detail-page enrichment additionally consumes `event_description.text` (from `/events/{id}/`), deliberately NOT `event_place.best_description` (that's the venue blurb, not the post body). Each event is split across TWO related nodes on the listing tab that share the same `id` — a "rich" node with `__typename:Event` (name/event_place) and a "time" node (start_timestamp/is_past). The parser merges by id; both halves must be present to emit. The required request headers are `User-Agent` (browser) + `Sec-Fetch-Dest: document` + `Sec-Fetch-Mode: navigate` + `Sec-Fetch-Site: none` — missing the `Sec-Fetch-*` triplet returns HTTP 400 on either listing or detail URLs. Reference: `src/adapters/facebook-hosted-events/`. Future-add data points (cover image, RSVP counts, end time, etc.) catalogued in `docs/event-schema-future-fields.md`.
  - **Datacenter-IP checkpoint → residential-proxy fallback (#1939):** FB now serves a checkpoint / "content isn't available" wall to *some* Pages from datacenter IPs (Vercel) — HTTP 200, zero `__typename:Event` nodes — so a direct fetch silently yields 0 events. `fetchFbListing` fetches direct first and, when the body `looksLikeFbBlock` (SSR envelope markers absent **or** a `/checkpoint/`/unavailable-content marker present) or the request 403s/throws, retries once through the NAS residential proxy (`safeFetch({ useResidentialProxy: true })`). `config.useResidentialProxy: true` skips the wasted direct attempt for chronically-blocked Pages (Survivor H3, Von Tramp). Detail-page fetches inherit the proxy decision. If both attempts stay blocked the adapter emits a checkpoint error → FAILED scrape (reconcile skipped), never a silent "0 events". The checkpoint is IP-specific and **cannot be reproduced from a dev box / the NAS residential IP** — verify the proxy leg on Vercel prod. `looksLikeFbBlock` lives in `parser.ts` (shared with the shape-break error).
  - **Soft-block (#2589):** FB also serves a *soft* block — HTTP 200 + a valid SSR envelope (`RelayPrefetchedStreamCache`/`__bbox`) + **zero Event nodes** — that `looksLikeFbBlock` does NOT catch (envelope present, no `/checkpoint/`). Prod evidence: this response is byte-for-byte structurally identical to a genuinely-empty Page (both ~610–630 KB, envelope present, no distinguishing HTML marker), so **no HTML heuristic can separate soft-block from empty**; the only real discriminator (a recent non-zero baseline) lives in the health layer, not the stateless adapter. `buildListingSignalErrors` therefore fails loud (→ FAILED, `SCRAPE_FAILURE`) on `0 events + envelope present + zero candidate nodes of any kind` **only for Pages flagged `useResidentialProxy: true`** (the "known datacenter-checkpoint-blocked" set — gsh3, vth3, dh4, soh4, Survivor H3). Non-flagged Pages keep SUCCESS-0 so a genuinely-empty Page never false-fails. Require *zero rejected candidates* too: if the parser rejected any bag (`cancelled`/`no-title`/`invalid-time`/`missing-half` — `admin-notice`/`placeholder` are the coverage-gap path), the events collection DID render, so it's not a block.
  - **Past-events backfill (#1940):** `config.includePastEvents: true` also fetches `/past_hosted_events` (best-effort — a past-tab failure is recorded in `diagnosticContext.pastFetch`, never added to `errors[]`, so it can't disable reconcile of future events). Reconcile-safe: `upcomingOnly: true` clamps reconcile's cancellation window to the future (reconcile.ts), so past events are enrichment-only and never cancelled; a past FB RawEvent (trust 8) overwrites a STATIC_SCHEDULE placeholder title/venue via merge's full-update branch. Past events use **listing-tab fields only** (no per-event detail enrichment — one extra GET/scrape), bounded by `config.pastWindowDays` (default 365). Combined upcoming+past events are deduped by FB event id (upcoming copy wins). Enabled on all FACEBOOK_HOSTED_EVENTS seed rows.
