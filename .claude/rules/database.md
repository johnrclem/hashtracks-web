...
- **Local dev DB:** For migration authoring, run a local Postgres 17 instance as a copy of Railway. See `.claude/rules/local-dev-db.md` for setup, refresh, and safety notes. The `scripts/safe-prisma.mjs` wrapper accepts localhost and is the only supported path for `migrate dev`.
