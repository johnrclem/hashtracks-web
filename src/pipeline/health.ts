...
/** A potential alert generated by a health check, pending dedup against existing open alerts. */
interface AlertCandidate {
  type: AlertType;
  severity: AlertSeverity;
  title: string;
  details: string;
  /** Structured metadata attached to the alert for admin UI display and repair actions. */
  context?: Record<string, unknown>;
}

/** Result of `analyzeHealth()`: overall source health status and any alerts to persist. */
...
/** Input data for `analyzeHealth()`, assembled by the scrape orchestrator after merge. */
...
/** Alert if the current scrape failed (adapter threw or returned errors). */
...
/** Escalate to CRITICAL if 3+ consecutive scrapes have failed. */
...
/** Alert for each field whose fill rate dropped >30pp below its rolling average (baseline ≥50%). */
...
/** Alert if HTML structural fingerprint changed; auto-resolves if hash stabilizes on next scrape. */
...
/** Alert if new kennel tags appeared that were not seen in recent scrapes. */
