...
describe("medical appointment filter (#1690)", () => {
  ...
  // Codex P1 review on PR #1713 (comment 3307074387) + claude-review:
  // the original `^\s*`-anchored patterns missed kennel-prefixed
  // appointment summaries (`"H4-TX: Sleep study"`) and any mid-string
  // medical phrase. Patterns now use `\b` word boundaries so the
  // prefix / mid-string shapes drop. The tighter override gate keeps
  // false-positives in check (themed hash trails have runNumber, hares,
  // or the `hash` keyword — all three override).
  it.each([
    // Kennel-prefixed (codex P1 verbatim shapes).
    "H4-TX: Sleep study",
    "Moooouston H3: Medical appointment with Dr. Smith",
    "GAL: Telehealth visit",
    // Mid-string medical phrase with no hash signal (claude-review).
    "Reminder - Sleep study tomorrow",
    "Note: Telehealth visit at 10am",
    // Sanity: the bare shape from the original issue still drops.
    "Sleep Study - Christine Kuhl Remote visit",
  ])("drops medical-pattern title regardless of position in summary: %s", (summary) => {
    const result = buildRawEventFromGCalItem(
      testGCalEvent({ summary }),
      { defaultKennelTag: "h4-tx" },
    );
