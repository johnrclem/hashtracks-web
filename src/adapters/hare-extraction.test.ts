...
/**
 * Run a table of (label, desc, expected) cases as `it.each` against extractHares.
 * Centralized so each describe block doesn't repeat the same `it.each(table)((_l, d, e) => …)` shell.
 */
function runExpected(template: string, table: ReadonlyArray<ExpectedRow>) {
  it.each(table.map((row) => [...row]))(template, (_label, desc, expected) => {
    expect(extractHares(desc as string)).toBe(expected);
  });
}

function runUndefined(template: string, table: ReadonlyArray<UndefinedRow>) {
  it.each(table.map((row) => [...row]))(template, (_label, desc) => {
    expect(extractHares(desc as string)).toBeUndefined();
  });
}

describe("extractHares — basic label patterns", () => {
  runExpected("extracts from %s", [
...
describe("extractHares — negative / filtered cases", () => {
  runUndefined("returns undefined for %s", [
...
describe("extractHares — embedded label truncation (HTML field collapse)", () => {
  runExpected("truncates at embedded %s label", [
...
describe("extractHares — boilerplate marker truncation", () => {
  ...
  runExpected("truncates at '%s' marker", [
...
describe("extractHares — trailing phone strip (formatted variants)", () => {
  runExpected("strips %s phone format", [
...
describe("extractHares — adversarial prose protection under label-only header", () => {
  runUndefined("%s does not pollute hares", [
...
describe("extractHares — WHO ARE THE HARES template (#774 BJH3)", () => {
  runExpected("captures %s from WHO ARE THE HARES: label", [
