...
describe("generateOccurrences", () => {
  ...
  // #1723: a kennel that runs the same monthly-nth weekday year-round but at a
  // different start time per season is encoded as two rules with the SAME BYDAY
  // and DISJOINT BYMONTH (so the rrule strings stay distinct — they can't collide
  // on @@unique([kennelId, rrule, source]) — while each carries its own startTime).
  // This locks the invariant the encoding relies on: the two month sets partition
  // the year, so every monthly occurrence is generated by exactly one rule, never
  // both. Mirrors the colh3 (1st Sunday) seasonal split.
  it("partitions monthly-nth occurrences across disjoint-BYMONTH seasonal rules (#1723)", () => {
    const start = utcDate(2026, 0, 1);
    const end = utcDate(2026, 11, 31, 23, 59, 59);
    const summer = generateOccurrences(
      parseRRule("FREQ=MONTHLY;BYDAY=1SU;BYMONTH=4,5,6,7,8,9"),
      start,
      end,
    );
    const winter = generateOccurrences(
      parseRRule("FREQ=MONTHLY;BYDAY=1SU;BYMONTH=1,2,3,10,11,12"),
      start,
      end,
    );

    // No date appears in both seasons.
    const overlap = summer.filter((d) => winter.includes(d));
    expect(overlap).toEqual([]);
    // Together they cover all twelve first Sundays of the year.
    expect(summer.length + winter.length).toBe(12);
    expect([...summer, ...winter].every((d) => new Date(d + "T12:00:00Z").getUTCDay() === 0)).toBe(true);
    for (const d of summer) expect([4, 5, 6, 7, 8, 9]).toContain(Number.parseInt(d.slice(5, 7), 10));
    for (const d of winter) expect([1, 2, 3, 10, 11, 12]).toContain(Number.parseInt(d.slice(5, 7), 10));
  });
});
